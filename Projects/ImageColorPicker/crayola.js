// Renard Augustin 20226233
// Seguin Pierre-Emmanuel 20164112
...
function createPie(data1) {
	...
	for (let i = 0; i < data1.length; i += 4) {
		couleur = data1.slice(i, i + 3)
		labelArray.push(names[getClosestColor(couleur)])
		colorArray.push(colors[getClosestColor(couleur)])
	}

	let colorSet = new Set(colorArray)
	let nameSet = new Set(labelArray)
	for (let uniqueColor of nameSet) {
		count.push(labelArray.filter(x => x == uniqueColor).length)
	}

	const dataPie = {
		labels: Array.from(nameSet),
		datasets: [{
			label: 'Répartition des pixels',
			data: count,
			backgroundColor: Array.from(colorSet).map(x => String(x)),
			hoverOffset: 4
		}]
	};
	const config = {
	...
	// J'ai fais la même méthode que dans la démo, mais il semble que Chart.js pose problème. 
	// Je fais bien un if statement avec tartExiste, si c'est vrais alors destroy() mon ancienne chart puis met tarteExiste sur false execute la chart et remet sur true...
	// Cela devrait permettre de supprimer la chart existance mais quand on rappuye sur lancer cela ne fonctionne pas
	if (tarteExiste) { myChart.destroy(); tarteExiste = false }
